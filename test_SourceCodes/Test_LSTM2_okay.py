...
# Sample CSV or database DataFrame
# Required columns: ProductName, Reviews, Rating (1–5)
# For demo, load a sample dataset
# df = pd.read_csv("reviews.csv")  # ← You can replace this line with your own data load
...
model = LSTMClassifier(len(vocab), embed_dim=64, hidden_dim=128).to(device)
...
def predict_product(product_name):
    ...
    with torch.no_grad():
        for _, row in related_reviews.iterrows():
            ...
            rating_tensor = torch.tensor([row['Rating']], dtype=torch.float).to(device)  # ✅ Include rating
...
user_input = input("\nEnter Product Name to Predict (e.g. 'Phone A'): ").strip()
