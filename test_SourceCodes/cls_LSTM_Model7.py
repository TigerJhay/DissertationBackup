...
nltk.download('punkt')  # Changed from punkt_tab
...
vocab = set()
for review in df['Reviews']: # Changed from reviews
...
print("word2idx saved to word2idx.json")
...
def encode_sentence(sentence):
    ...
    ids = [word2idx.get(token, 0) for t in tokens][:MAX_LEN]
    return ids + [0] * (MAX_LEN - len(ids))
...
def calculate_accuracy(outputs, targets):
    ...
    Args:
        outputs (torch.Tensor): Model's raw output (probabilities).
    ...
    correct = (predicted == targets).float().sum()  # Count correct predictions
...
for epoch in range(10):
    ...
    for x, rating, y in train_loader:
        ...
        train_correct += (train_acc * y.size(0)).int().item()
        train_total += y.size(0)

        print(f"Epoch {epoch + 1}, Loss: {total_loss:.4f}, Train Accuracy: {train_correct / train_total:.4f}")

    # Calculate average training loss and accuracy for the epoch
    avg_train_loss = total_loss / len(train_loader)
    avg_train_accuracy = train_correct / train_total

    print(f"Epoch {epoch + 1}, Average Training Loss: {avg_train_loss:.4f}, Average Training Accuracy: {avg_train_accuracy:.4f}")
    ...
    with torch.no_grad():  # Disable gradient calculation during evaluation
        for x, rating, y in test_loader:
            ...
            test_acc = calculate_accuracy(output, y)
            test_correct += (test_acc * y.size(0)).int().item()
            test_total += y.size(0)
    ...
    print(f"Epoch {epoch + 1}, Average Test Loss: {avg_test_loss:.4f}, Average Test Accuracy: {avg_test_accuracy:.4f}")
